locals {
  ...
  use_default_registry = length(var.service_registries) == 0 || !var.enable_service_registry
  default_registry = [
    {
  ...
  service_registries = local.use_default_registry ? local.default_registry : var.service_registries
}
...
resource "aws_ecs_task_definition" "default" {
  ...
  container_definitions    = var.container_definition_json != null ? var.container_definition_json : jsonencode([
  ...
  tags = var.use_old_arn ? null : merge(module.this.tags, { "service-type" = var.service_type })
}
...
resource "aws_iam_role" "ecs_task" {
  ...
  name                 = "${module.task_label.id}-esc-task-role"
...
resource "aws_iam_role" "ecs_service" {
  ...
  name                 = "${module.service_label.id}-ecs-service-role"
...
resource "aws_iam_role_policy" "ecs_service" {
  count  = local.enable_ecs_service_role && var.service_role_arn == null ? 1 : 0
  name   = "${module.service_label.id}-ecs-service-policy"
...
resource "aws_iam_role_policy" "ecs_ssm_exec" {
  count  = local.create_task_role && var.exec_enabled ? 1 : 0
  name   = "${module.task_label.id}-ecs-ssm-exec-policy"
...
resource "aws_iam_role" "ecs_exec" {
  count                = local.create_exec_role ? 1 : 0
  name                 = "${module.exec_label.id}-esc-exec-role"
...
resource "aws_iam_role_policy" "ecs_exec" {
  ...
  name     = "${module.exec_label.id}-ecs-exec-policy"
...
resource "aws_iam_role" "ecs_service_connect_tls" {
  count                = local.create_service_connect_tls_role ? 1 : 0
  name                 = "${module.service_connect_label.id}-ecs-connect-role"
...
resource "aws_ecs_service" "ignore_changes_task_definition" {
  ...
  dynamic "load_balancer" {
    ...
    content {
      ...
      target_group_arn = lookup(load_balancer.value, "target_group_arn") != null ? lookup(load_balancer.value, "target_group_arn") : aws_alb_target_group.fargate_alb_target_group[load_balancer.value.target_group_index].arn
    }
  }
...
resource "aws_ecs_service" "ignore_changes_task_definition_and_desired_count" {
  ...
  dynamic "load_balancer" {
    ...
    content {
      ...
      container_port   = load_balancer.value.container_port
      elb_name         = lookup(load_balancer.value, "elb_name", null)
      target_group_arn = lookup(load_balancer.value, "target_group_arn") != null ? lookup(load_balancer.value, "target_group_arn") : aws_alb_target_group.fargate_alb_target_group[load_balancer.target_group_index].arn
    }
  }
